...
- Ruby 3.2+（開発は 3.4.10 で確認。`.ruby-version` 参照）、Bundler
...
- 退勤 = max(所定時刻, 最終業務イベントの終了時刻)＋揺らぎ。会食・懇親会など `calendar.exclude_keywords` に一致する末尾イベントは飛ばして判定。ただし揺らぎを足すと日付を跨ぐ日（例: 23:59 終了のイベント）は揺らぎを落として基準時刻そのものを目標にする。日付が変わると当日の計画は破棄されるため、翌日に出た目標は必ず未打刻になり、23:59:59 で頭を押さえるだけでも tick（既定30秒）が日付変更までに入らず打刻機会をほぼ失うため。
...
- pmset の起床予約を自動管理（add-only 方式）: 当日の残り打刻分と翌営業日朝（`daemon.morning_wake_at` と所定出勤時刻の早い方）のぶんを、毎回 `pmset -g sched` を読んで不足している起床だけ追加する（sudoers 設定時。`manage_wake: false` で無効化可）。予約には登録元 `ak4-punch` を付け（`pmset -g sched` に `by 'ak4-punch'` と表示される）、その予約だけを自分のものとして突き合わせる。`cancelall`/`cancel` は使わない。これらはマシン全体の一回限り起床予約を消してしまい、登録元を付けずに登録された予約はどれも `by 'pmset'` と表示されて区別できないため、同居する他の pmset 利用デーモン（capital-arena の仮想取引デーモン）の起床予約まで壊してしまうため。何も消さないので他デーモンと共存でき、逆に他デーモンの `cancelall` で自分の予約が消えても次のポーリングで再追加され自己回復する。
...
（打刻の寝過ごしスキップ / リトライ失敗 / トークン再発行失敗 / sukesan 障害フォールバック / 休暇の時間帯に入ったための打刻中止（出勤・退勤とも）/ 日中の再取得・退勤直前チェックで全休へ切り替わったための以降の打刻中止 / 本日の打刻計画の作成に失敗 / 起床予約の処理（pmset）が戻らない）。未設定なら無効です。
`SLACK_MENTION`（例: `<@U04XXXXXX>`）を設定すると通知の先頭にメンションを付けます（任意）。

送信に失敗した通知は同一プロセスのメモリに保持し、以降の tick で自動再送します（スリープ明け直後で Wi-Fi 未接続でも、復旧すれば送られます）。
保持できるのは10件までで、超えた分は古いものから破棄します。デーモンを再起動すると保留分は失われます。
