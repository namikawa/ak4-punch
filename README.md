...
- 出勤目標の更新で、新しい目標が既に「目標＋`late_grace_minutes`」を過ぎている場合（例: 9:40 に 9:00 開始の会議が入った）は更新せず、既存の目標を維持する。採用した瞬間に窓超過と判定され、出勤が恒久的にスキップされてしまうため。逆に窓内（例: 9:03 に 9:00 開始の会議が入った）なら新しい目標を採用し、その場で打刻する。
