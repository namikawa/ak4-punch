...
AKASHI の打刻API は、リクエストの `stampedAt`（打刻日時）を無視し、APIを呼び出した瞬間のサーバ時刻で記録します（実機検証済み。公式仕様の注記どおり）。
公開APIに「任意の時刻を実績として後から書き込む」手段は存在しません。

➡ そのため本ツールは「打刻したい時刻に、その打刻APIを呼ぶ」方式を採ります。
出勤時刻（例 9:30）と退勤時刻（例 18:00）に cron で `punch` を2回起動します。記録時刻＝実行時刻（秒単位のブレのみ）。
